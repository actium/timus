...
namespace long_arithmetic {
    ...
    private:
        ...
        static void subtract(storage_type& lhs, const storage_type& rhs)
        {
            add(lhs, rhs.size(), is_negative(rhs) ? 0 : ~uint32_t(0), [&](size_t i) {
                return ~rhs[i];
            }, 1);
        }
...
int main()
{
    ...
    long_arithmetic::integer u = 1, v = 0;

...
{
    return P[n] * E[k] * E[n-k];
}

unsigned solve(std::vector<unsigned>& s, unsigned m)
{
    precompute(m);
    ...
    for (auto it = s.begin(); it != s.end(); t += *it++) {
        std::vector<integer> ndp(1+m);
        for (unsigned i = (t < m ? m - t : 0); i <= m && t <= 2 * (m - i); ++i) {
            const unsigned j = 2 * (m - i) - t;

            const unsigned lb = (*it > j ? *it - j : 0), ub = std::min(*it, i);
            for (unsigned q = lb; q <= ub; ++q)
                ndp[i-q] += dp[i] * C(i, q) * C(j, *it - q);
        }
        std::swap(dp, ndp);
    }
